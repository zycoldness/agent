...
1. **Track A，SingGuard 机制复现**：无外部工具、无多轮环境。模型在单次调用中直接接收完整 active policy、素材和输出要求，复现运行时策略输入、fast/hybrid/slow 推理和规则命中。
...
5. **Oracle Evaluator**：独立保存最终标签、规则 ID 和最短充分轨迹，用于奖励与离线评测，绝不提供给工具。
...
| `final_decision` | `label`, `rule_id`, `confidence` | 终止状态 | `rule_id` 必须属于当前 policy |

Agent 可在任一轮调用 `final_decision`。环境在达到三轮、模型输出截断、非法调用或完成最终动作后终止。Track A 不启用任何工具；其 fast/hybrid/slow 是一次调用内的解码路径，不是多轮 Agent 行为。

最终输出统一为：`label`、`risk_level`、`rule_id`、`policy_version`、`confidence`、`route` 和 `next_action`。不保存自由 CoT；证据 ID 仅用于 Evidence Store 内部定位，不作为模型最终输出。
...
| `oracle` | 最终标签、命中规则、最短充分轨迹 | 否 | SFT 标签、奖励和评测 |
...
4. **B1，多轮 Tool-SFT**：基于人工确认的专家轨迹学习工具格式、停止时机与证据使用，只针对需要外部取证的难例。
...
`R = 1.0 * label_correct + 1.0 * rule_correct + 0.2 * useful_tool - 0.08 * tool_calls - 0.5 * invalid_action`
...
- `invalid_action`：非法 JSON、未知工具、错误策略版本或不合法 rule ID。
...
继续进入多轮 RL 的门槛：Track A2 在动态策略反事实集上首先达到预设 policy-following 门槛。继续进入 4B、更多轮或 OPD 的门槛：Track B 相对于 Track A slow guard，在固定白样本误伤上限内，提高规则 ID exact match 或最终 F1，且平均工具次数与 P95 延迟处于预设预算之内。
...
| reward hacking | 终局正确、规则、成本和非法动作组合奖励；报告动作分布 |
